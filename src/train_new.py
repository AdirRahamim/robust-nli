...
def get_args():
    ...
    parser.add_argument("--outputmodelname", type=str, default='model.new')
    parser.add_argument("--outputmodelname_adversary", type=str, default='model_adv.pickle')
    parser.add_argument("--train_lbls_file", type=str, default='../data/snli_1.0/cl_snli_train_lbl_file',
                        help="NLI train data labels file")
    parser.add_argument("--train_src_file", type=str, default='../data/snli_1.0/cl_snli_train_source_file',
                        help="NLI train data source file")
    parser.add_argument("--val_lbls_file", type=str, default='../data/snli_1.0/cl_snli_val_lbl_file',
                        help="NLI validation (dev) data labels file")
    parser.add_argument("--val_src_file", type=str, default='../data/snli_1.0/cl_snli_val_source_file',
                        help="NLI validation (dev) data source file")
    parser.add_argument("--test_lbls_file", type=str, default='../data/snli_1.0/cl_snli_test_lbl_file',
                        help="NLI test data labels file")
    parser.add_argument("--test_src_file", type=str, default='../data/snli_1.0/cl_snli_test_source_file',
                        help="NLI test data source file")
    ...
    parser.add_argument("--remove_dup", type=int, default=0,
                        help="Whether to remove hypotheses that are duplicates. 0 for no, 1 for yes (remove them).")
    ...
    # alpha - method 1
    parser.add_argument("--adv_hyp_encoder_lambda", type=float, default=1.,
                        help="adversarial weight for hypothesis only encoder")
    # beta - method 2
    parser.add_argument("--nli_net_adv_hyp_encoder_lambda", type=float, default=0.5,
                        help="adversarial weight for hypothesis encoder in NLI net")
    # alpha - method 1
    parser.add_argument("--random_premise_frac", type=float, default=0.,
                        help="fraction of random premises to use in NLI net")
...
def trainepoch(epoch, train, optimizer_nli, params, word_vec, shared_nli_net,
               loss_fn_nli, adv_lambda, adv_hyp_encoder_lambda):
    ...
    for stidx in range(0, len(hypoths), params.batch_size):
        ...
        premises_batch_orig, premises_len_orig = get_batch(premises[stidx:stidx + params.batch_size], word_vec)
        premises_batch_rand, premises_len_rand = get_batch(["."] * hypoths_batch.shape[1], word_vec)

        tgt_batch = None
        if params.gpu_id > -1:
            hypoths_batch = Variable(hypoths_batch.cuda())
            premises_batch_orig = Variable(premises_batch_orig.cuda())
            premises_batch_rand = Variable(premises_batch_rand.cuda())
            tgt_batch = Variable(torch.LongTensor(target[stidx:stidx + params.batch_size])).cuda()
        else:
            hypoths_batch = Variable(hypoths_batch)
            premises_batch_orig = Variable(premises_batch_orig)
            premises_batch_rand = Variable(premises_batch_rand)
            tgt_batch = Variable(torch.LongTensor(target[stidx:stidx + params.batch_size]))

        k = hypoths_batch.size(1)  # actual batch size

        # model forward
        output_nli = shared_nli_net((premises_batch_orig, premises_len_orig), (hypoths_batch, hypoths_len), False)
        ...
        # Another forward pass with random premise
        output_nli = shared_nli_net((premises_batch_rand, premises_len_rand), (hypoths_batch, hypoths_len), True)
        ...
        if len(all_costs_nli) == 100:
            logs.append(
                '{0} ; loss nli {1} ; sentence/s {2} ; words/s {3} ; accuracy train nli {4}'.format(
    ...
    print(
        'results : epoch {0} ; mean accuracy train nli : {1}, loss nli : {2}'
            .format(epoch, train_acc_nli, round(np.mean(all_costs_nli), 2), ))
    return train_acc_nli, shared_nli_net


def evaluate(epoch, valid, optimizer_nli, params, word_vec, shared_nli_net, eval_type='valid', final_eval=False,
             adv_lambda=1.):
    shared_nli_net.eval()
    correct_nli = 0.
    ...
    if final_eval:
        print('finalgrep : accuracy {0} : nli {1}'.format(eval_type, eval_acc_nli))
    else:
        print('togrep : results : epoch {0} ; mean accuracy {1} : nli {2}'.format(epoch, eval_type, eval_acc_nli))
...
def main(args):
  ...
  """
    ...
    word_vecs = build_vocab(
        train['hypoths'] + val['hypoths'] + test['hypoths'] + train['premises'] + val['premises'] + test['premises'],
    ...
    elif "spr" in lbls_file or "dpr" in lbls_file or "fnplus" in lbls_file or "add_one" in lbls_file or "scitail" in lbls_file or "toy" in lbls_file:
  ...
  """
    ...
    while not stop_training and epoch <= args.n_epochs:
        train_acc_nli, shared_nli_net = trainepoch(epoch, train, optimizer_nli, args, word_vecs, shared_nli_net,
                                                   loss_fn_nli, args.adv_lambda, args.adv_hyp_encoder_lambda)
        eval_acc_nli = evaluate(epoch, val, optimizer_nli, args, word_vecs, shared_nli_net, 'valid',
